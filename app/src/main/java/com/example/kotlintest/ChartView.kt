     ...
     //   for (i in 1 until chartCount+1){
       ...
       var i=list.indexOf(c)+1
        ...
        var charRealHeigth=(height-marg*2)- fl +marg
             canvas?.drawRect(marg+chartsMarg*i+charWidth*(i-1),charRealHeigth,marg+chartsMarg*i+charWidth*i,height-marg,paintXY)
            canvas?.drawText(c.xName,marg+chartsMarg*i+charWidth*(i-1)+15,height-marg+20,paintXY)
            canvas?.drawText(charRealHeigthValue.toString(),marg+chartsMarg*i+charWidth*(i-1)+15,charRealHeigth-5,paintXY)
        }
